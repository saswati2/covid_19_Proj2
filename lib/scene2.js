...
const renderScene2 = (svgId, data) => {
  ...
  const yAxisMaxVal = d3.max(data, d => {
    return Math.max(d.Brazil, d.India, d.France, d.Germany, d.US);
  });
  ...
  const colorValue = ['Brazil', 'India', 'France', 'Germany', 'US'];
  const transitionDuration = 50600;

  const colorScale = d3.scaleOrdinal()
    .domain(colorValue)
    .range(['green', '#fdae61', 'purple', '#2c7bb6', '#d7191c']);  
...
d3.csv("data/transposed_covid_data.csv").then(rawData => {
  data = rawData.map(d => ({
    ...
    !isNaN(d.Time) && !isNaN(d.US) && !isNaN(d.Germany) && !isNaN(d.France) && !isNaN(d.India) && !isNaN(d.Brazil)
  );
