...
const renderScene2 = (svgId, data) => {
  ...
  const waveAnnotations = [
    { date: new Date('2020-06-24'), label: '2nd Wave' },
    { date: new Date('2020-11-02'), label: '3rd Wave' },
    { date: new Date('2021-12-10'), label: '4th Wave' }
  ];

  const waveCountries = {
    '2nd Wave': ['US', 'India', 'France', 'Germany', 'Brazil'],
    '3rd Wave': ['US', 'India', 'France', 'Germany', 'Brazil'],
    '4th Wave': ['US', 'India', 'France', 'Germany', 'Brazil']
  };

  waveAnnotations.forEach(annotation => {
    const annotationDate = annotation.date;
    const annotationLabel = annotation.label;
    const annotationCountries = waveCountries[annotationLabel];
    const x1 = xScale(annotationDate);
    const yValues = annotationCountries.map(country => ({
      country: country,
      y: yScale(data.find(d => d.Time >= annotationDate)[country])
    }));
    const y = d3.mean(yValues, d => d.y);

    g.merge(gEnter).append('rect')
      .attr('transform', `translate(${x1 + 20},${y - 20})`)
    ...
    g.merge(gEnter).append('text')
      .attr('transform', `translate(${x1 + 25},${y - 5})`)
