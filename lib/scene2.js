...
const renderScene2 = (svgId, data) => {
  ...
  // Wave dates
  const waveDates = [
    { date: new Date('2020-06-24'), label: '2nd Wave', countries: { US: true, India: true, Germany: true, France: true, Brazil: true } },
    { date: new Date('2020-11-02'), label: '3rd Wave', countries: { US: true, India: true, Germany: true, France: true, Brazil: true } },
    { date: new Date('2021-12-10'), label: '4th Wave', countries: { US: true, India: true, Germany: true, France: true, Brazil: true } }
  ];

  waveDates.forEach((wave, i) => {
    const annotationX = xScale(wave.date);

    Object.keys(wave.countries).forEach(country => {
      if (wave.countries[country]) {
        const annotationY = yScale(data.find(d => d.Time.toDateString() === wave.date.toDateString())[country]);
        ...
        gEnter.append('line')
          ...
          .attr('x2', annotationX)
          .attr('y1', annotationY)
          .attr('y2', annotationY - 180)  // 180 pixels above the curve
          .attr('stroke', 'black')
          .attr('stroke-width', 2)
          .attr('stroke-dasharray', '4,4');

        const textBoxPadding = 8;
        const text = `${wave.label} in ${country}`;
        const textWidth = 200;
        const textHeight = 24;

        gEnter.append('rect')
          .attr('x', annotationX - textWidth / 2 - textBoxPadding)
          .attr('y', annotationY - textHeight - 180 - textBoxPadding)
