...
const renderScene2 = (svgId, data) => {
  ...
  //3rd annotation
  const annotationDate2 = new Date('2021-05-06');
  const annotationX2 = xScale(annotationDate2);
  const annotationY2 = yScale(data.find(d => d.Time.toDateString() === annotationDate2.toDateString()).India);
  ...
  // Rectangle with text
  const textBoxPadding2 = 8;
  const text2 = "Middle of 3rd wave caused by Delta Varient in India";
  const textWidth2 = 300;
  const textHeight2 = 40;
