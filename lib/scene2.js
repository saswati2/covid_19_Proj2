...
const colorLegend = (selection, props) => {
  ...
  selection.append('g')
    .attr('class', 'show-all')
    .attr('transform', `translate(0, ${colorScale.domain().length * spacing})`)
    .append('text')
    .text('Show All')
    .attr('dy', '0.32em')
    .attr('x', textOffset)
    .on('click', () => {
      activeCountries = new Set(colorScale.domain());
      renderScene2("#scene2-svg", data);
    });
};
...
const renderScene2 = (svgId, data) => {
  ...
  Object.keys(lineGenerators).forEach((country, i) => {
    g.merge(gEnter).append('path')
      ...
      .attr('opacity', activeCountries.has(country) ? 0.7 : 0)
  ...
  // ... (your annotations code remains the same)
...
d3.csv("data/transposed_covid_data.csv").then(rawData => {
  ...
  activeCountries = new Set(colorValue);
  renderScene2("#scene2-svg", data);
});
