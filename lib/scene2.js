...
const renderScene2 = (svgId, data) => {
  ...
  waveAnnotationsGrouped.forEach(group => {
    const annotationPoints = group.values.map(d => {
      const point = data.find(p => p.Time.getTime() === d.date.getTime());
      if (!point) {
        console.error(`Date ${d.date} not found in data.`);
        return null;
      }
      return {
        x: xScale(d.date),
        y: yScale(point[d.countries[0]])
      };
    }).filter(d => d !== null);
    ...
    group.values.forEach(d => {
      ...
      gEnter.merge(g).append('line')
        ...
        .attr('y1', yScale(point[d.countries[0]]))
        .attr('x2', annotationX)
        .attr('y2', annotationY)
        .attr('stroke', 'black')
        .attr('stroke-width', 1);
