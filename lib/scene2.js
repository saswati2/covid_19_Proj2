...
const renderScene2 = (svgId, data) => {
  ...
  const annotationDate1 = new Date('2022-01-01');
  ...
  const text1 = "Spike of Cases caused by Omicron Variant";
  const textWidth1 = 160;
  const textHeight1 = 50;
  ...
  const annotationY2 = yScale(d3.max(data, d => d.US)); // Adjust y position
  ...
  gEnter.append('rect')
    ...
    .attr('y', annotationY2 - textHeight2 - textBoxPadding2)
  ...
  gEnter.append('text')
    .attr('x', annotationX2)
    .attr('y', annotationY2 - textBoxPadding2)
    .attr('text-anchor', 'middle')
    .attr('font-size', '12px')
    .attr('font-weight', 'bold')
    .text(text2);
