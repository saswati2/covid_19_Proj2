...
const renderIndexPage = (svgId, data, metric) => {
  ...
  const nested = d3.nest()
    ...
    .rollup(function (d) {
      return {
        latestConfirmedCases: d3.sum(d, function (e) { return e["3/9/23"]; }),
        latestDeaths: d3.sum(d, function (e) { return e["3/9/23"]; })
      };
    })
    .entries(data);
  ...
  xAxisG.append('text')
    ...
    .attr('x', innerWidth / 2)
    .attr('fill', 'black')
    .text(metric === "cases" ? 'Number of Confirmed Cases as on 3/09/2023' : 'Number of Deaths as on 3/09/2023');
  ...
  const mouseleave = function (d) {
    ...
    d3.select(this)
      .style("stroke", "none")
      .style('fill', metric === "cases" ? 'steelblue' : 'pink');
  }
...
d3.csv('data/time_series_covid19_confirmed_global_latest.csv').then(data => {
  ...
  d3.csv('data/time_series_covid19_deaths_global_latest.csv').then(data => {
    dataDeaths = data;
    renderIndexPage('#index-svg', dataCases, "cases");
    ...
    d3.select("#metric-select").on("change", function () {
      ...
      if (selectedMetric === "cases") {
        ...
        renderIndexPage('#index-svg', dataDeaths, "deaths");
      }
    });
  });
});
